-- 0061_coins_separate_balance_and_redeem.sql
--
-- New coins model: coins are their own balance, separate from wallet
-- rupees. 1 coin = ₹1. Customer redeems manually from Profile once
-- balance ≥ 100. No auto-deposit at order_place anymore.
--
-- Schema:
--   wallets.coins_balance — currently-spendable coins (0+).
--   wallets.coins_lifetime stays as historical counter (display only).
--
-- order_place no longer adds coins to balance_paise. Just increments
-- coins_balance + coins_lifetime + writes a 'coins_credit' transaction.
--
-- Customers who already had cashback auto-deposited under the old
-- model keep those rupees in balance_paise — no migration. Going
-- forward, every new order's cashback accumulates in coins_balance.
--
-- New RPC coins_redeem(p_amount): caller-invoked. Requires p_amount ≥ 100.
-- Atomically: deducts coins_balance, credits balance_paise by the same
-- integer amount (1 coin = 1 rupee), writes 'coins_debit' transaction.

ALTER TABLE wallets
  ADD COLUMN IF NOT EXISTS coins_balance INTEGER NOT NULL DEFAULT 0
    CHECK (coins_balance >= 0);

COMMENT ON COLUMN wallets.coins_balance IS
  'Currently-spendable Diaries Coins. 1 coin = 1 rupee on redeem. Earned via order cashback, redeemed via coins_redeem RPC (min 100).';
...
BEGIN
  ...
  -- Coins are now whole-rupee-equivalent integers (NOT paise). Divide
  -- by 100 to convert paise → rupees, then apply cashback %.
  v_coins := (v_cashback_eligible_net * v_config.cashback_percent / 100 / 100)::INTEGER;
  ...
  IF p_payment_method = 'wallet' THEN
    ...
    UPDATE wallets SET
      balance_paise = balance_paise - v_total,
    ...
    INSERT INTO wallet_transactions(
      ...
      p_family_id, 'order_debit', -v_total, v_wallet.balance_paise,
...
CREATE OR REPLACE FUNCTION coins_redeem(p_amount INTEGER)
RETURNS JSONB
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
...
BEGIN
  ...
  IF p_amount IS NULL OR p_amount < 100 THEN
    RAISE EXCEPTION 'min_redeem_100';
  END IF;

  SELECT * INTO v_wallet FROM wallets
    WHERE family_id = v_caller_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'wallet_not_found'; END IF;
  IF v_wallet.coins_balance < p_amount THEN
    RAISE EXCEPTION 'insufficient_coins';
  END IF;

  v_paise := p_amount * 100;

  UPDATE wallets SET
    coins_balance = coins_balance - p_amount,
    balance_paise = balance_paise + v_paise,
    updated_at = now()
  WHERE family_id = v_caller_id RETURNING * INTO v_wallet;

  INSERT INTO wallet_transactions(
    family_id, type, amount_paise, balance_after_paise,
    coins_amount, payment_method
  ) VALUES (
    v_caller_id, 'coins_debit', v_paise, v_wallet.balance_paise,
    -p_amount, 'system'
  );

  INSERT INTO audit_log(actor_id, actor_type, action, entity_type, entity_id, new_value)
  VALUES (
    v_caller_id, 'customer', 'coins.redeem', 'wallet', v_wallet.id,
    jsonb_build_object(
      'coins_redeemed', p_amount,
      'paise_credited', v_paise,
      'balance_paise_after', v_wallet.balance_paise,
      'coins_balance_after', v_wallet.coins_balance
    )
  );
  ...
  RETURN jsonb_build_object(
    ...
    'coins_redeemed', p_amount,
    'paise_credited', v_paise,
    'balance_paise', v_wallet.balance_paise,
    'coins_balance', v_wallet.coins_balance
  );
END $$;

REVOKE EXECUTE ON FUNCTION coins_redeem(INTEGER) FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION coins_redeem(INTEGER) TO authenticated;
