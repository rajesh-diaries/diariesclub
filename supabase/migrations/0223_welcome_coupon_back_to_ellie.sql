-- Welcome coupon code should be the hero-themed ELLIE (not WELCOME100).
-- Keeps min_kids (1) and discount (₹100) unchanged; only the code + the RPC
-- lookup change. Final coupon set is fully hero-themed:
--   ELLIE(1) ₹100, ZENA(2) ₹150, RAFI(3) ₹250, GERRY(4) ₹400, WILDPACK(5) ₹500.
UPDATE coupons SET code='ELLIE' WHERE upper(code)='WELCOME100';
...
BEGIN
  ...
  SELECT * INTO v_coupon FROM coupons WHERE upper(code) = 'ELLIE';
  IF NOT FOUND
