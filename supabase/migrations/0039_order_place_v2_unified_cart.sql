-- ===========================================================================
--  Migration 0039 — order_place v2: unified heterogeneous cart (Module 2.5/2.6)
--
--  Resolves the split-cart problem from Module 2.5: FIT meals previously
--  bypassed the cart and went straight to fit_meal_orders. With the
--  client-side heterogeneous cart now in place, order_place handles all
--  three line types in a single transaction.
--
--  Wire format change for p_items:
--    Each entry has a `type` discriminator:
--      { "type": "menu_item", "menu_item_id": "<uuid>", "quantity": N }
--      { "type": "combo",     "combo_id": "<uuid>",     "quantity": N }
--      { "type": "fit_meal",  "template_id": "<uuid>",  "quantity": N,
--        "selections": { "<cat_id>": "<opt_id>", ... } }
--
--  Backward compat: entries lacking a `type` field default to 'menu_item'
--  (matches the legacy shape). p_combo_id parameter is now ignored —
--  combos arrive as line items. Kept in signature for back-compat.
--
--  Schema additions:
--    - order_items.line_type CHECK ('menu_item' | 'combo' | 'fit_meal')
--    - order_items.combo_id (nullable FK)
--    - order_items.fit_meal_order_id (nullable FK)
--    - order_items.menu_item_id loosened to NULL-allowed (combo/fit_meal lines have no menu_item)
--    - order_items.selections_jsonb for fit_meal lines (display reproducibility)
--    - fit_meal_orders.order_id linkage
--
--  Pricing model: each line is independent. No combo-discount math —
--  combo's price is just the combo's price (admin already discounted vs.
--  à-la-carte). Subtotal = Σ(line.unit_price × quantity). GST applied to
--  subtotal as before.
--
--  Reversibility:
--    -- Restore legacy order_place body from 0003:601.
--    ALTER TABLE order_items
--      DROP COLUMN IF EXISTS line_type,
--      DROP COLUMN IF EXISTS combo_id,
--      DROP COLUMN IF EXISTS fit_meal_order_id,
--      DROP COLUMN IF EXISTS selections_jsonb,
--      ALTER COLUMN menu_item_id SET NOT NULL;
--    ALTER TABLE fit_meal_orders DROP COLUMN IF EXISTS order_id;
-- ===========================================================================

BEGIN;

-- 1. order_items extensions
ALTER TABLE order_items
  ADD COLUMN IF NOT EXISTS line_type TEXT NOT NULL DEFAULT 'menu_item'
    CHECK (line_type IN ('menu_item','combo','fit_meal')),
  ADD COLUMN IF NOT EXISTS combo_id UUID REFERENCES combos(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS fit_meal_order_id UUID REFERENCES fit_meal_orders(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS selections_jsonb JSONB;

-- Loosen menu_item_id; combo and fit_meal lines won't have one.
DO $$ BEGIN
  ALTER TABLE order_items ALTER COLUMN menu_item_id DROP NOT NULL;
EXCEPTION WHEN others THEN NULL;
END $$;

-- 2. fit_meal_orders extension — link to parent order.
ALTER TABLE fit_meal_orders
  ADD COLUMN IF NOT EXISTS order_id UUID REFERENCES orders(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_fit_meal_orders_order
  ON fit_meal_orders(order_id) WHERE order_id IS NOT NULL;
...
CREATE OR REPLACE FUNCTION order_place(
  ...
  p_combo_id UUID DEFAULT NULL,        -- ignored; kept for back-compat
...
BEGIN
  ...
  -- Pass 1: validate every line and accumulate subtotal.
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    ELSIF v_type = 'combo' THEN
      ...
      v_subtotal := v_subtotal + (v_combo.price_paise * v_qty);
    ELSIF v_type = 'fit_meal' THEN
      v_fit_priced := _fit_validate_and_price(
        (v_item->>'template_id')::UUID,
        v_item->'selections'
      );
      v_subtotal := v_subtotal + ((v_fit_priced->>'final_price_paise')::INTEGER * v_qty);
    ELSE
  ...
  -- Insert orders row. combo_id + combo_discount_paise stay 0/NULL in v2 —
  -- combos are line items now, no global discount math.
  INSERT INTO orders(
  ...
  -- Pass 2: snapshot every line into order_items.
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    v_type := COALESCE(v_item->>'type', 'menu_item');
    v_qty := (v_item->>'quantity')::INTEGER;
    ...
    ELSIF v_type = 'fit_meal' THEN
      ...
      -- Insert fit_meal_orders linked to this order.
      ...
      -- Snapshot in order_items for unified order rendering.
      INSERT INTO order_items(
