-- 0153 — Workshop registration confirmation push
--
-- Customer feedback (2026-05-17): after registering for a workshop, the app
-- shows only a brief snackbar and pops the screen. No push notification, no
-- persistent in-app confirmation. Parents have no record of the registration
-- until they navigate back to the workshops list.
--
-- This migration:
--   1. Adds a 'workshop_registered' notification template.
--   2. Patches workshop_register RPC to fire the notification + write to
--      the in-app notifications inbox on every successful registration
--      (idempotent path skips the push — already-registered families
--      shouldn't re-notify).

INSERT INTO notification_templates(type, enabled, title, body, deep_link_template)
VALUES (
  'workshop_registered',
  true,
  'You''re in! 🎨',
  '{{child_name}} is registered for {{title}} on {{scheduled_date}}.',
  '/profile/workshops'
)
...
CREATE OR REPLACE FUNCTION public.workshop_register(p_workshop_id uuid, p_family_id uuid, p_child_id uuid, p_payment_method text, p_idempotency_key text DEFAULT NULL::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- BUG-071: send confirmation push + in-app notification.
