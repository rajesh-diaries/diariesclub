-- Add batch_mode to qr_scan_validate so one QR scan can check in
-- multiple kids when a parent selected several children in the same
-- purchase. Backwards-compatible: batch_mode defaults to false,
-- preserving today's single-session behaviour.
--
-- When batch_mode = true and the primary session was pending, we also
-- activate any other pending sessions for the same family at the same
-- venue that were created within the last 5 minutes (the "same batch"
-- window). This prevents accidentally activating old stuck sessions.
...
BEGIN
  ...
  -- Trust the QR payload's batch_mode flag if the caller didn't explicitly
  -- pass p_batch_mode. This makes the staff app's job easier — it can just
  -- forward the raw QR payload without decoding it client-side.
  ...
  -- Primary session notification + audit
  ...
  -- Primary session goes into the results array first.
