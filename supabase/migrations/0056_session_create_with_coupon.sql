-- 0056_session_create_with_coupon.sql
--
-- Lets a customer apply a coupon at session_create time. The wallet
-- hold (or cash amount) is the discounted total, not the full price.
--
-- Schema:
--   * sessions.coupon_redemption_id — nullable FK to coupon_redemptions.
--     Set when a coupon was applied at create time. Useful for refund /
--     audit / future un-redeem logic.
--
-- session_create gains a `p_coupon_code TEXT DEFAULT NULL` parameter.
-- If supplied, validates inline (mirrors coupon_redeem checks), computes
-- the discount, deducts it from v_amount, then writes the redemption
-- row + sets coupon_redemption_id on the session — all in the same
-- transaction.
--
-- Cancelling a pending session does NOT auto-un-redeem the coupon for
-- v1 simplicity. The redemption stays on record. If a customer cancels
-- after applying, they've used that coupon (per family quota stands).

ALTER TABLE sessions
  ADD COLUMN IF NOT EXISTS coupon_redemption_id UUID
    REFERENCES coupon_redemptions(id) ON DELETE SET NULL;

DROP FUNCTION IF EXISTS session_create(UUID,UUID,UUID,INTEGER,TEXT,UUID,BOOLEAN,TEXT,UUID,TEXT);
...
BEGIN
  ...
  -- Coupon validation + discount calculation. Lock the coupon row so
  -- two concurrent redemptions can't oversubscribe a max_uses cap.
  ...
  -- Record coupon redemption + link from session row.
...
GRANT EXECUTE ON FUNCTION session_create(UUID,UUID,UUID,INTEGER,TEXT,UUID,BOOLEAN,TEXT,UUID,TEXT,TEXT)
