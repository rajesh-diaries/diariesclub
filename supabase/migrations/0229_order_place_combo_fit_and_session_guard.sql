-- 0229 — order_place: charge/record combo FIT upcharge + true one-session guard
--
-- Bug A (undercharge + lost kitchen data): order_preview prices a combo that
-- has fit_template_id (requires fit_selections, adds the FIT upcharge to
-- food_taxable). order_place did NONE of this — it charged the base combo
-- only, never recorded the FIT build, so FIT-combos (live: ₹1000/₹1100) were
-- undercharged by the option upcharges and the kitchen never saw the choices.
-- Fix: mirror order_preview's FIT pricing, and insert a fit_meal_orders row +
-- selections on the combo order_item so the KDS gets the build.
--
-- Bug B (overcharge): the "one session per order" guard only rejected qty>1 on
-- a single combo line. Two separate session-combo lines (each qty=1) both added
-- session value to the total while only ONE sessions row is created → customer
-- charged for two sessions, receives one. Fix: count session units across ALL
-- lines and reject when >1.
--
-- Everything else is reproduced verbatim from the live def (2026-07-04).

CREATE OR REPLACE FUNCTION public.order_place(
  p_venue_id uuid, p_family_id uuid, p_items jsonb, p_fulfillment_mode text,
  p_payment_method text, p_combo_id uuid DEFAULT NULL::uuid, p_staff_pin_id uuid DEFAULT NULL::uuid,
  p_child_id uuid DEFAULT NULL::uuid, p_idempotency_key text DEFAULT NULL::text,
...
DECLARE
  ...
  v_session_line_count INTEGER := 0;
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    ELSIF v_type = 'combo' THEN
      ...
      -- FIT-linked combo: validate + price the selections and charge the
      -- upcharge (mirrors order_preview so preview == charge).
      ...
      IF v_combo_session_minutes IS NOT NULL AND v_combo_session_minutes > 0 THEN
        ...
        END IF;
        -- One play session per order (one child, one QR). Count across ALL
        -- lines so two separate session-combos, or qty>1, both reject.
        v_session_line_count := v_session_line_count + v_qty;
        IF v_session_line_count > 1 THEN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    ELSIF v_type = 'combo' THEN
      ...
      IF v_combo.fit_template_id IS NOT NULL THEN
        -- Record the FIT build for the kitchen and stamp the combo line with
        -- its per-unit charged price (base + upcharge) and the selections.
        v_fit_priced := _fit_validate_and_price(
          v_combo.fit_template_id,
          v_item->'fit_selections'
        );
        ...
        INSERT INTO fit_meal_orders(
          ...
          p_family_id, v_combo.fit_template_id,
          (v_fit_priced->>'base_price_paise')::INTEGER,
          v_combo_fit_upcharge,
          (v_fit_priced->>'final_price_paise')::INTEGER,
          v_item->'fit_selections', 'ordered', v_order.id
        );
        ...
        INSERT INTO order_items(
          ...
          v_qty, v_combo.price_paise + v_combo_fit_upcharge, v_item->'fit_selections'
        );
      ELSE
        INSERT INTO order_items(
          order_id, line_type, combo_id, brand, name_snapshot,
