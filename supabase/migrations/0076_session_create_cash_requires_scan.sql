-- 0076 — customer-initiated cash sessions must wait for staff QR scan
--
-- Bug: session_create routed wallet→pending and cash→active. A
-- customer who picked 'Cash at venue' got a session with status='active'
-- and timer counting down even before they arrived at the venue and
-- before any cash changed hands. If they never showed up, the session
-- ran out the clock for free.
--
-- Fix: any customer-initiated session (p_staff_pin_id IS NULL) goes
-- to 'pending' regardless of payment method. Staff QR scan flips it
-- to 'active' and starts the real timer; for cash, scan = cash
-- collected at counter. session-autocancel-pending-cron already cleans
-- up no-shows after the pre-scan timeout window.
--
-- v_use_hold semantics unchanged — still wallet-specific, controlling
-- the held_paise vs. immediate-debit logic. New v_pending_scan flag
-- separately controls status routing.
...
BEGIN
  ...
  -- Wallet hold (held_paise vs. immediate debit) is still wallet-specific.
  v_use_hold := (p_payment_method = 'wallet' AND p_staff_pin_id IS NULL);
  -- Status routing: ANY customer-initiated session must wait for staff
  -- QR scan before going active, regardless of payment method.
