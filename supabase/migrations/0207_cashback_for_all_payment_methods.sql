-- 0207 — cashback (coins) should be earned on all payment methods, not just wallet.
--
-- Previously order_place only credited coins when p_payment_method = 'wallet'.
-- Customers paying at counter (cash) or via Razorpay still earned coins on the
-- receipt, but the coins were never added to wallets.coins_balance. This
-- migration moves the coins credit out of the wallet-only block so it runs for
...
BEGIN
  ...
  -- Wallet debit only when paying with wallet.
  ...
  -- Coins are earned regardless of payment method.
  IF v_coins > 0 THEN
    SELECT * INTO v_wallet FROM wallets WHERE family_id = p_family_id FOR UPDATE;
...
REVOKE EXECUTE ON FUNCTION public.order_place(
  uuid, uuid, jsonb, text, text, uuid, uuid, uuid, text, text
) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.order_place(
  uuid, uuid, jsonb, text, text, uuid, uuid, uuid, text, text
