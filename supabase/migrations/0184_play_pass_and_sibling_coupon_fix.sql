-- Consolidated fix: applies all missing Play Pass + sibling coupon pieces
-- that previous migrations (0176-0182) may not have landed due to partial
-- applies, silent EXCEPTION swallowing, or being skipped entirely.
-- Everything here is idempotent — safe to re-run.

-- ===========================================================================
-- 1. Sibling auto-coupons (from 0176)
-- ===========================================================================
...
-- ===========================================================================
-- 2. Play Passes table + RLS (from 0182)
-- ===========================================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'play_passes' AND policyname = 'play_passes_family_select'
  ) THEN
    CREATE POLICY "play_passes_family_select"
      ON public.play_passes FOR SELECT TO authenticated
      USING (family_id = auth.uid());
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'play_passes' AND policyname = 'play_passes_family_insert'
  ) THEN
    CREATE POLICY "play_passes_family_insert"
      ON public.play_passes FOR INSERT TO authenticated
      WITH CHECK (family_id = auth.uid());
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'play_passes' AND policyname = 'play_passes_family_update'
  ) THEN
    CREATE POLICY "play_passes_family_update"
      ON public.play_passes FOR UPDATE TO authenticated
      USING (family_id = auth.uid()) WITH CHECK (family_id = auth.uid());
  END IF;
END $$;

-- ===========================================================================
-- 3. Safari notice config columns (from 0182)
-- ===========================================================================
...
-- ===========================================================================
-- 4. Fixed play_pass_purchase function (from 0182)
-- ===========================================================================
...
BEGIN
  ...
  FROM wallet_transactions WHERE family_id = p_family_id;
...
-- ===========================================================================
-- 5. play_pass_redeem function (from 0182)
-- ===========================================================================
...
BEGIN
  ...
  WHERE id = p_pass_id AND family_id = p_family_id FOR UPDATE;
  ...
  UPDATE play_passes SET used_passes = used_passes + 1 WHERE id = p_pass_id;
...
-- ===========================================================================
-- 6. session_create with play_pass + coupon support (from 0182)
-- ===========================================================================
