-- 0208 — Safari waitlist stores child DOB; fix admin package audit_log columns

-- Customer app now captures date of birth instead of (just) an age chip.
ALTER TABLE public.safari_waitlist
  ADD COLUMN IF NOT EXISTS child_dob DATE;

-- The audit_log schema was normalized in 0094. A few admin package functions
-- still referenced the old (actor_user_id, entity, payload) shape and fail
-- whenever an admin creates or updates a birthday package.
DROP FUNCTION IF EXISTS public.admin_package_create(
  ...
  integer, integer, integer, text, jsonb, text, text, text, text
);
DROP FUNCTION IF EXISTS public.admin_package_update(
  uuid, text, text, text, integer, integer, integer, integer, integer,
  text, text[], jsonb, jsonb, jsonb, jsonb, text, boolean, integer, text,
  integer, integer, integer, integer, text, jsonb, text, text, text, text
);
...
BEGIN
  ...
  INSERT INTO birthday_packages(
    ...
    hero_theme, sort_order, is_active, category,
    ...
    p_hero_theme, p_sort_order, true, p_category,
  ...
  VALUES (
    auth.uid(), 'admin', 'package.create', 'birthday_package', v_id, p_venue_id,
...
BEGIN
  ...
  SELECT venue_id INTO v_venue_id FROM birthday_packages WHERE id = p_id;
  IF v_venue_id IS NULL THEN RAISE EXCEPTION 'package_not_found'; END IF;
  ...
  INSERT INTO audit_log(actor_id, actor_type, action, entity_type, entity_id, venue_id, new_value)
  VALUES (
    auth.uid(), 'admin', 'package.update', 'birthday_package', p_id, v_venue_id,
    jsonb_build_object('name', p_name, 'tier', p_tier, 'hall_name', p_hall_name)
  );
