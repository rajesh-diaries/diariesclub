-- 0059_order_place_gst_inclusive.sql
--
-- Architecture rule (per migration 0011 + CLAUDE.md): all in-app prices
-- are GST-INCLUSIVE. Menu UI already shows 'incl. GST' below every
-- price. order_place v2 was treating prices as GST-EXCLUSIVE — adding
-- 18% on top — so customers were billed Subtotal ₹180 + GST ₹32 = ₹212
-- for an admin-priced ₹180 item.
--
-- Fix: compute v_gst as the embedded portion of the gross subtotal,
-- not as an addition. v_total stays equal to v_subtotal (the price the
-- customer was shown). Order receipt still breaks out GST for
-- accounting / audit, but the customer pays only the displayed price.
--
-- Math: gross = net + gst, gst = gross * pct / (100 + pct).
--   gross ₹180, pct 18 → gst = 180 * 18 / 118 ≈ ₹27.46
--                       net = 180 - 27.46 ≈ ₹152.54
--
-- Cashback now computes on the NET subtotal (food/drink value) rather
-- than the gross — customers don't earn coins on the GST portion.
--
-- Reversibility: re-deploy 0039_order_place_v2_unified_cart.sql.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    ELSIF v_type = 'fit_meal' THEN
      ...
      v_gross := v_gross + ((v_fit_priced->>'final_price_paise')::INTEGER * v_qty);
    ELSE
