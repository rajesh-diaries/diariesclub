-- Enforce coupon.min_kids at session creation (the authoritative charge point):
-- a multi-kid coupon can only be applied when the batch has at least that many
-- kids. The client passes p_kid_count = number of children in the booking.
DROP FUNCTION IF EXISTS public.session_create(uuid,uuid,uuid,integer,text,uuid,boolean,text,uuid,text,text,uuid);
