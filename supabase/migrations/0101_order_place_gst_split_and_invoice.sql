-- 0101 — order_place RPC: full GST split + invoice numbering
-- Food: 5% on top of pre-GST price (HSN 9963)
-- Sessions: 18% inclusive in display price, back-calculated for tax
--           reporting (HSN 9996)
-- Combos: combo total = session portion (18% incl) + food portion
--         (pre-GST). 5% added to the food portion only.
...
BEGIN
  ...
  -- ─── First pass: split each line into food_taxable vs session_value
  ...
  -- Cashback: on the food taxable portion (pre-GST). Coins in whole rupees.
  ...
  -- ─── Generate invoice number + insert order
...
-- Drop the old signature (without p_customer_gstin) so PostgREST
-- doesn't get a duplicate-overload error.
DROP FUNCTION IF EXISTS public.order_place(
  uuid, uuid, jsonb, text, text, uuid, uuid, text
);
