-- 0226 — Fix duplicate stage-perk grants (cross-trait fan-out)
--
-- Bug: xp_credit_with_split() grants one stage-perk row per trait that crosses
-- a stage boundary, but the perk-lookup inside the `FOREACH v_trait` loop filters
-- by STAGE only:
--     SELECT ... FROM stage_perks
--      WHERE stage = v_step_stage AND is_active = true
--        AND (venue_id IS NULL OR venue_id = p_venue_id)
-- It is missing `AND trait = v_trait`. So for every trait that transitions, it
-- grants EVERY trait's perk for that stage, tagging each with the current trait.
-- A child whose 4 traits all reach Seedling in one session gets 4 traits × 4
-- seedling perks = 16 identical "Seedling sticker" grants instead of 4 (one per
-- character). Confirmed in prod: child "Jr Raj" had 16 seedling grants, and the
-- 4 rows tagged trait='rafi' pointed at 4 different perk_ids (ellie/gerry/rafi/
-- zena's perks). The sibling helper _grant_stage_perk_slot() already filters by
-- trait correctly; this inline loop did not.
--
-- Fix: add the trait predicate so each transitioning trait only grants its own
-- perk(s). Every active stage_perk has a non-null trait, and this loop only ever
-- runs for stages seedling..legend (welcome perks are handled elsewhere), so the
-- filter never excludes a legitimate perk. The child's persisted stage_* columns
-- already prevent re-granting on later sessions, so no extra dedup is needed.
--
-- Only the perk SELECT changes; the rest of the function is reproduced verbatim
-- from prod (captured 2026-07-04).

CREATE OR REPLACE FUNCTION public.xp_credit_with_split(
  p_child_id uuid, p_family_id uuid, p_venue_id uuid, p_event_type text,
  p_xp_rafi integer DEFAULT 0, p_xp_ellie integer DEFAULT 0,
  p_xp_gerry integer DEFAULT 0, p_xp_zena integer DEFAULT 0,
  p_reference_id uuid DEFAULT NULL::uuid, p_metadata jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
        ...
        FOR v_perk_row IN
          SELECT id, perk_label, validity_days
           ...
           WHERE stage = v_step_stage
             AND is_active = true
             AND trait = v_trait                              -- FIX: only this trait's perk(s)
             AND (venue_id IS NULL OR venue_id = p_venue_id)
        LOOP
