-- 0054_session_complete_referral_hook.sql
--
-- After a session completes, if the family was referred by someone AND
-- this is their first ever completed session AND the conversion hasn't
-- fired yet, call referral_convert to credit both wallets.
--
-- referral_convert raises on failure (monthly cap, etc). We swallow that
-- inside session_complete so a failed referral never blocks the actual
-- session close — staff and customer flows must not be affected.
--
-- Mirror copy of 0047 + new hook block at the end. Reversible by
-- re-deploying 0047 verbatim.
...
BEGIN
  ...
  -- Referral conversion hook (added 0054).
  -- Fires once per family on their first completed session, if a
  -- referrer was attached via referral_attach. Wrapped in BEGIN/EXCEPTION
  -- so a failed conversion (monthly cap, anomaly) never blocks session
  -- completion.
  ...
  IF v_family.referrer_family_id IS NOT NULL THEN
    ...
    IF v_is_first_session AND NOT EXISTS (
