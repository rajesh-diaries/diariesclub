...
/// Realtime list of menu items for one brand. Reads from the `menu_items`
/// table and filters to the requested brand via a one-time lookup of
/// `menus(id, brand)`. The initial one-shot query returns all columns so
/// the customer UI can display tags, symbols, offer pricing, prep time,
/// and availability.
///
    ...
    StreamProvider.family<List<Map<String, dynamic>>, String>((
      ...
      try {
        ...
        await for (final rows in stream) {
          yield rows.where((r) => brandMap[r['menu_id']] == brand).toList();
        }
      } catch (e) {
        // ignore: avoid_print
